            ...
            <template slot-scope="props">
              <el-form label-position="left" inline class="demo-table-expand">
                ...
                <el-form-item label="头像">
                  <span style="width: 40px;height: 40px;"><img :src="server+props.row.avatar" alt="" width="40px" height="40px" style="border-radius: 20px"></span>
                </el-form-item>
                ...
                <el-form-item label="性别">
                  <span>{{ props.row.sex?props.row.sex:'未填写'}}</span>
                </el-form-item>
                <el-form-item label="生日">
                  <span>{{ props.row.birthday?props.row.birthday:'未填写' }}</span>
                </el-form-item>
                <el-form-item label="密码">
                  <span>{{ props.row.password }}</span>
                </el-form-item>
                <el-form-item label="个人签名">
                  <span>{{ props.row.sign?props.row.sign:'未填写' }}</span>
                </el-form-item>
              </el-form>
            </template>
          </el-table-column>
      ...
      <div>
        ...
        >
          <el-form :label-position="labelPosition" :rules="rules" label-width="80px" :model="userInfo">
            ...
            <el-form-item label="头像">
              <el-col :span="16">
                <img :src="userInfo.avatar?userInfo.avatar:server+'/images/avatar/monkey.png'" ref="previewImg" alt="" width="80px" height="80px">
                <div style="position: relative;height: 64px">
                  ...
                  <div slot="tip" class="el-upload__tip" style="position: absolute;left: 0;top: 32px;height: 32px">只能上传jpg/png文件，且不超过500kb（默认为系统头像）</div>
                </div>
              </el-col>
            </el-form-item>
            ...
            <el-form-item label="生日">
              <el-col :span="16">
                <el-date-picker type="date" placeholder="选择日期" v-model="userInfo.birthday" :picker-options="pickerBirthday" style="width: 100%;"></el-date-picker>
              </el-col>
            </el-form-item>
            <el-form-item label="个人签名">
              <el-col :span="16">
                <el-input type="textarea" v-model="userInfo.sign" :autosize="{ minRows: 2, maxRows: 4}"></el-input>
              </el-col>
            </el-form-item>
          </el-form>
...
<script>
    import {getCurrentPageUser,upLoadImg,updateUserInfo,deleteUserInfo,addUserInfo} from '../../../api/index'
    ...
    export default {
        ...
        methods: {
          ...
          handleDelete(index, row) {
            MessageBox.confirm('此操作将永久删除该用户所有信息, 是否继续？','提示').then(async (value)=>{
              if (value==='confirm'){
          ...
          async manageUserInfo(){
            if (!this.userInfo.user_name||!(/^1[3|4|8|6|7|8][0-9]{9}$/.test(this.userInfo.phone))||!this.userInfo.password) {
              ...
              if (json.success_code===200){
                ...
                if (this.dialogTitle === '编辑用户信息') {
                  json = await updateUserInfo(this.userInfo.user_id,this.userInfo.user_name,this.userInfo.avatar,this.userInfo.password,this.userInfo.sex,this.userInfo.phone,this.userInfo.sign,this.userInfo.birthday);
                  if (json.success_code===200){
                  ...
                  json = await addUserInfo(this.userInfo.user_name,this.userInfo.avatar,this.userInfo.phone,this.userInfo.password,this.userInfo.sex,this.userInfo.sign,this.userInfo.birthday);
                  if (json.success_code===200){
                    this.loadCurrentPageUser(this.currentPage,8,this.searchInput);
                    this.dialogFormVisible = false;
